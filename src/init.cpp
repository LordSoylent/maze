...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MAZE.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MAZEd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMAZEamount=<n> " + _("Keep N MAZE anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MAZE is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MAZE is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MAZE version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MAZE server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MAZE") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MAZE to complete") << "\n";
    ...
    nAnonymizeMAZEAmount = GetArg("-anonymizeMAZEamount", 0);
    if(nAnonymizeMAZEAmount > 999999) nAnonymizeMAZEAmount = 999999;
    if(nAnonymizeMAZEAmount < 2) nAnonymizeMAZEAmount = 2;
    ...
    LogPrintf("Anonymize MAZE Amount %d\n", nAnonymizeMAZEAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MAZE+1000 == (.1MAZE+100)*10
       10MAZE+10000 == (1MAZE+1000)*10
    */
