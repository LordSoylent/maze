...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMAZEAmount"))
        settings.setValue("nAnonymizeMAZEAmount", 1000);
    nAnonymizeMAZEAmount = settings.value("nAnonymizeMAZEAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMAZEAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMAZEAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMAZEAmount:
            return QVariant(nAnonymizeMAZEAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMAZEAmount:
            nAnonymizeMAZEAmount = value.toInt();
            settings.setValue("nAnonymizeMAZEAmount", nAnonymizeMAZEAmount);
            emit AnonymizeMAZEAmountChanged(nAnonymizeMAZEAmount);
            break;
        default:
            break;
        }
