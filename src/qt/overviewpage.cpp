...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMAZEAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMAZEAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMAZEAmount = strAnonymizeMAZEAmount.remove(strAnonymizeMAZEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMAZEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMAZEAmount*COIN) nMaxToAnonymize = nAnonymizeMAZEAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMAZEAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMAZEAmount));
        strAnonymizeMAZEAmount = strAnonymizeMAZEAmount.remove(strAnonymizeMAZEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMAZEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
